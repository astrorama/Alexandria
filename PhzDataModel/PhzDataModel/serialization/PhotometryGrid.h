...
void save(Archive& ar, const PhzDataModel::PhotometryGrid& grid, const unsigned int) {
...
template<typename Archive>
void load(Archive& ar, PhzDataModel::PhotometryGrid& grid, const unsigned int) {
  std::vector<std::string> filter_names;
  ar >> filter_names;
  ...
  for (auto& cell : grid) {
    ...
    cell = ChCatalog::Photometry {filter_names_ptr, std::move(phot_values)};
  }
}
...
template<typename Archive>
void serialize(Archive& ar, PhzDataModel::PhotometryGrid& t, const unsigned int version) {
