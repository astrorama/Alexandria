...
BOOST_FIXTURE_TEST_CASE(Constructor, Piecewise_Fixture) {
  ...
  for (double knot : knots) {
    functions.push_back(std::shared_ptr<ChMath::Function>(new IntegrableMock(knot)));
  }
  functions.pop_back();
...
BOOST_FIXTURE_TEST_CASE(ConstructorUnorderedKnots, Piecewise_Fixture) {
  ...
  // Then
  BOOST_CHECK_THROW(ChMath::Piecewise(knots, functions), ElementsException);
...
BOOST_FIXTURE_TEST_CASE(Clone, Piecewise_Fixture) {
  ...
  ChMath::Piecewise* resPiece = dynamic_cast<ChMath::Piecewise*>(clonePtr.get());
