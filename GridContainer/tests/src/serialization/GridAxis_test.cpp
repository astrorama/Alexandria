...
BOOST_AUTO_TEST_CASE(fundamentalKnotValues) {
  ...
  Grid::GridAxis<double>* axis_ptr = &axis;
  boa << axis_ptr;
  Grid::GridAxis<double>* result;
  boost::archive::binary_iarchive bia {stream};
  bia >> result;
  // We use a unique_ptr for the memory management
  std::unique_ptr<Grid::GridAxis<double>> result_ptr {result};
...
BOOST_AUTO_TEST_CASE(defaultConstructibleKnotValues) {
  ...
  Grid::GridAxis<DefaultConstructibleClass> axis {name, knots};
  ...
  // We write to the stream a pointer to enable the non-default constructor
  // functionality of boost serialization
  Grid::GridAxis<DefaultConstructibleClass>* axis_ptr = &axis;
  boa << axis_ptr;
  Grid::GridAxis<DefaultConstructibleClass>* result;
  boost::archive::binary_iarchive bia {stream};
  bia >> result;
  // We use a unique_ptr for the memory management
  std::unique_ptr<Grid::GridAxis<DefaultConstructibleClass>> result_ptr {result};
...
BOOST_AUTO_TEST_CASE(nonDefaultConstructibleKnotValues) {
  ...
  Grid::GridAxis<NonDefaultConstructibleClass> axis {name, knots};
  ...
  // We write to the stream a pointer to enable the non-default constructor
  // functionality of boost serialization
  Grid::GridAxis<NonDefaultConstructibleClass>* axis_ptr = &axis;
  boa << axis_ptr;
  Grid::GridAxis<NonDefaultConstructibleClass>* result;
  boost::archive::binary_iarchive bia {stream};
  bia >> result;
  // We use a unique_ptr for the memory management
  std::unique_ptr<Grid::GridAxis<NonDefaultConstructibleClass>> result_ptr {result};
