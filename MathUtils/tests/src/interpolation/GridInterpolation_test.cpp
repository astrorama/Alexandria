...
BOOST_AUTO_TEST_CASE(SingleDiscreteValue) {
  ...
  InterpN<double, EnumType> interp({knots0, knots1}, values, true);
  ...
  // Swap axes
  InterpN<EnumType, double> interp2({knots1, knots0}, values.reshape(1, 4), true);
...
*/
  // Swap axes
  InterpN<EnumType, double> interp2({knots1, knots0}, values.reshape(3, 1), true);
  //BOOST_CHECK_CLOSE(interp2(EnumType::A, 1.), 2., 1e-8);
...
BOOST_AUTO_TEST_CASE(BadConstructor) {
  ...
  // Bad grid shape
  try {
    InterpN<double, EnumType> x({knots0, knots1}, values, true);
  ...
  // Knots not sorted
  try {
    InterpN<double, double> x({std::vector<double>{2, 0}, std::vector<double>{1, 2}}, values, true);
    BOOST_FAIL("Constructor should have thrown");
  } catch (const InterpolationException&) {
  }
  ...
  try {
    InterpN<double, double, double> x{{std::vector<double>{1, 2}, std::vector<double>{3, 4}, std::vector<double>{1}}, values, true};
    BOOST_FAIL("Constructor should have thrown");
  } catch (const InterpolationException&) {
  }

  try {
    InterpN<double> x{{std::vector<double>{1, 2, 3, 4}}, values, true};
    BOOST_FAIL("Constructor should have thrown");
  } catch (const InterpolationException&) {
  }
  ...
  try {
    InterpN<double, double> x{{std::vector<double>{1, 2, 3}, std::vector<double>{5, 6}}, values, true};
    BOOST_FAIL("Constructor should have thrown");
  } catch (const InterpolationException&) {
  }
}
