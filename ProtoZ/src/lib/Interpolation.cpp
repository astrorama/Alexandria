...
BaseInterpolation::BaseInterpolation(const vector<double>& x, const vector<double>& y, int m)
                   : xx(x), yy(y), m_x_size(x.size()), m_x_subset_size(m), m_j_saved(0), m_hunt_or_locate(0)
{
...
int BaseInterpolation::locate(const double x)
{
  ...
  {
    throw("BaseInterpolation::locate: locate size error");
  }
...
int BaseInterpolation::hunt(const double x)
{
  ...
  if (m_x_size < 2 || m_x_subset_size < 2 || m_x_subset_size > m_x_size)
    throw("BaseInterpolation::hunt: size error");
...
void CubicSplineInterpolation::sety2(const std::vector<double>& xv, const std::vector<double>& yv, double yp1, double ypn)
{
  int i,k;
  double p,qn,sig,un;
  ...
  for (i = 1; i < n-1; i++) {
    ...
    if (p) {
       ...
       throw("CubicSplineInterpolation::sety2: division by zero!!!");
    }
...
double CubicSplineInterpolation::rawinterp(const int jl, const double x)
{
  ...
  if (h == 0.0)
    throw("CubicSplineInterpolation::rawinterp: bad input to routine, h=0!!");
